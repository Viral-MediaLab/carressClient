/*
Modified code to show whether touch is working or not, etc.
*/
...
$(document).ready(function() {
  ...
  addStyleString("div.touch-circle{ border-radius: 50%; width: 40px; height: 40px; margin-left: -20px; margin-top: -20px; z-index: 1000000000000; position: absolute; background-color: rgba(0,0,0,0.3); pointer-events: none; top: -300px; left: -300px; }");
  ...
  var div = $("<div/>").addClass("touch-circle");
  $('body').append(div);

  window.connectTouch(5000);

  return;
});

window.connectTouch = function(port) {
  try{
    ...
    console.log("COULD NOT CONNECT TO TOUCH");
    return false;
  }
...
var div = $("<div/>").addClass("touch-status");

CSS for touch status WIP:

.touch-status{
  transition: width 0.5s ease 0.2s, height 0.2s ease, color 0.1s ease, padding 0s ease 0.7s;
  position: absolute;
  left: 50px;
  top: 50px;
  width:0px;
...
.touch-status.shown{
    ...
    color: rgba(0,0,0,1);
    padding-left: 2vw;
    transition: width 0.5s ease 0.2s, height 0.2s ease,color 0.2s ease 0.7s, padding 0s ease 0s;
}
